...
import CriteriaLoadingSkeleton from "~web/domains/criteria/components/skeletons/CriteriaLoadingSkeleton.vue";
import {
  CriteriaResponse,
  criteriaService,
  Criterion,
...
const { execute, data, loading, genericError } = useApi<CriteriaResponse>();
...
<template>
  <div class="mx-auto w-full max-w-6xl space-y-6 pb-24">
    ...
    <div v-if="!!genericError" class="flex h-full items-center justify-center">
      <EmptyOrErrorComponent :icon="ListTree" />
    </div>

    <CriteriaLoadingSkeleton v-else-if="loading || !data" />

    <div
      v-else-if="data.criteria.length === 0"
      class="flex h-full items-center justify-center"
    >
      <EmptyOrErrorComponent
        :icon="ListTree"
        custom-title="Aucun critère de notation défini"
        custom-desc="Aucun critère n'a encore été défini. Commencez par créer votre premier critère."
      />
    </div>
      ...
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        >
          ...
          >
            ...
            <ButtonComponent
              variant="danger"
              @click="openDeleteModal(criterion)"
            >
              <Trash class="h-4 w-4" />
            </ButtonComponent>
          </div>
